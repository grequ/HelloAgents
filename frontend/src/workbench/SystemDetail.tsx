...
function generateAdditionalContext(sys: System, ucs: UseCase[]): string {
  ...
  if (highFreq.length > 0) {
    lines.push(`\nExpected traffic: ${highFreq.map((u) => `${u.name} (${u.frequency})`).join(", ")}. Consider caching or batching for high-volume calls.`);
  }
              ...
              {(allUseCasesBySystem[la.systemId] || []).length > 0 && (
                <div className="flex flex-wrap gap-1.5 ml-1">
                  {(allUseCasesBySystem[la.systemId] || []).map((uc) => (
                    <label key={uc.id} className={`text-xs px-2 py-1 rounded border cursor-pointer transition-colors ${la.useCaseIds.includes(uc.id) ? "border-tedee-cyan bg-tedee-cyan/10 text-tedee-navy" : "border-gray-200 bg-white text-gray-500"}`}>
                      <input type="checkbox" className="hidden" checked={la.useCaseIds.includes(uc.id)} onChange={() => toggleUseCase(list, setList, idx, uc.id)} />
                      {uc.name}
                    </label>
                  ))}
                </div>
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
        ...
        <div className="mb-3">
          ...
          <AutoTextarea className={inp} value={genConfig.agent_role} onChange={(v) => setGenConfig({ ...genConfig, agent_role: v })} placeholder="How should this agent behave?" />
        </div>
        <div>
          <label className="block text-xs text-gray-500 mb-1">Additional Context</label>
          <AutoTextarea className={inp} value={genConfig.additional_context} onChange={(v) => setGenConfig({ ...genConfig, additional_context: v })} placeholder="Business rules, compliance requirements, rate limits..." />
        </div>
      </div>
