...
async def ensure_schema():
    ...
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            ...
            # Create agent specs table if missing
            await cur.execute("""
                CREATE TABLE IF NOT EXISTS wb_agent_specs (
                    id CHAR(36) PRIMARY KEY, name VARCHAR(255) NOT NULL,
                    agent_ids JSON, use_case_ids JSON,
                    spec_markdown LONGTEXT, tools_json JSON,
                    system_prompt LONGTEXT, skeleton_code LONGTEXT,
                    depends_on JSON, called_by JSON,
